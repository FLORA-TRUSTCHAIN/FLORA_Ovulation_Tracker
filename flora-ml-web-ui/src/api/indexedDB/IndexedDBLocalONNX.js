...
export const getFileFromIndexedDB = async (filename) => {
  try {
    const db = await initDB(OBJECT_STORE_NAME_LOCAL);
    const transaction = db.transaction([OBJECT_STORE_NAME_LOCAL], 'readonly');
    const store = transaction.objectStore(OBJECT_STORE_NAME_LOCAL);
